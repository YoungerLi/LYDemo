...
1. 新建合并请求前自己先做一下 review，消除掉警告和错误不影响合并后的编译运行。一个合并请求当包含 0.5-1 天的工作量，以便提高 review 的效率，

2. 新建的合并请求可以指派任意一人（包括自己），在描述中列出需要大家 review 的地方，并 @iOS 组，为了及时进行 review 可以在 QQ 或当面通知大家。

3. 每个成员都要做 review，无论大小问题当需指出来，不确定的可以面对面讨论出结果。确定没有问题后在评论中回复 *LGTM* (looks good to me)，有问题立即修正。若时间上来不及需要 review 者在 iOS/MonkeyKing 上提交一个 to do 的 issue 并指派给对方。

4. 当所有人都觉得没有问题并回复了 LGTM 后，合并请求才可以被同意合并到 dev 分支。
...
.m 文件中的代码用 ``` #pragma mark - Your Group Name``` 分组，顺序依照如下：
...
- (void)setYourProperty:(id) property {}
- (id)yourProperty {}
